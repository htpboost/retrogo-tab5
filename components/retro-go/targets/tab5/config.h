 ...
 ****************************************************************************/
// Tab5 は物理ゲームパッド非搭載。
// 暫定: GT911 タッチ(画面ゾーン→キー)で操作。drivers/input/tab5_touch.h を rg_input が取り込む。
//   上端→UP / 下端→DOWN / 左端→SELECT(前タブ) / 右端→START(次タブ) / 中央→A(決定)
// 本命: USB-A ホストの HID ゲームパッド(BSP 依存に usb_host_hid あり)を後で追加予定。
#define RG_GAMEPAD_TOUCH_TAB5  1
// #define RG_GAMEPAD_GPIO_MAP { ... }  // 将来 USB HID 等に置換
